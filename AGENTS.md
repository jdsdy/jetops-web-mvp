...
- `lib/personal.ts` — personal account paths, route access, and API auth guard (`requirePersonalAccount`).
- `lib/fleet.ts` — fleet aircraft helpers: reference grouping, payload validation, organisation and personal fleet queries, active-member auth for fleet reads.
- `lib/flights.ts` — flight helpers: storage path building, PDF validation, organisation and personal flight queries; bucket `flight_plan_pdfs`.
...
- Tests in `tests/lib/auth.test.ts`, `tests/lib/organisation.test.ts`, `tests/lib/personal.test.ts`, `tests/lib/fleet.test.ts`, and `tests/lib/flights.test.ts`.
- Small API helpers (e.g. `jsonError`) live inline in the route file that uses them, not separate lib files.
- Organisation API routes under `app/api/organisations/[organisationId]/members`, `.../invites`, `.../fleet`, and `.../flights`; personal API routes under `app/api/personal/fleet` and `app/api/personal/flights`; plus `GET /api/aircraft-reference` and `POST /api/invites/store-cookie`, `POST /api/invites/consume-cookie`; one resource path per route file with HTTP verbs; session auth → membership/admin or personal account check → user Supabase client when RLS allows, `createAdminClient` when RLS blocks; all handlers wrap with `withApiLogging` from `lib/api-logging.ts`.
...
- Personal portal under `app/app/personal/(portal)/`: `(portal)/layout.tsx` auth-gates via `resolvePersonalRouteAccess`; sections Flights (default) and Fleet; flight analysis at `app/app/personal/flights/`; shell in `components/personal-app-shell.tsx`; reuses organisation section and analysis components with personal API paths.
...
- Callback loaders: `/app/callback` and `/auth/accept-invite` show `CallbackLoader` while resolving; `/auth/callback` uses route-local `AuthCallbackLoader` with the same shared component. Feature docs: `documentation/authentication.md`, `documentation/onboarding.md`, `documentation/routes.md`, `documentation/api-logging.md`, `documentation/personal-app.md`, `documentation/organisation-app.md`, `documentation/organisation-invites.md`, `documentation/organisation-members.md`, `documentation/fleet.md`, `documentation/flights.md`, `documentation/landing-page.md`.
- Unit tests use Vitest (`npm test`); domain helpers are tested in `tests/lib/auth.test.ts`, `tests/lib/organisation.test.ts`, `tests/lib/personal.test.ts`, `tests/lib/fleet.test.ts`, `tests/lib/flights.test.ts`, and `tests/lib/api-logging.test.ts`.
