...
- Account types are `organisation` or `personal`; signup stores `account_type` in user metadata and a trigger creates `profiles`; signup codes validated via JetOps API (`GET {JETOPS_API_URL}/v1/signup`, `lib/auth.ts` `validateSignupCode`).
...
- Callback loaders: `/app/callback` shows `CallbackLoader` while resolving membership; `/auth/callback` uses route-local `AuthCallbackLoader` with the same shared component. Feature docs: `documentation/authentication.md`, `documentation/onboarding.md`, `documentation/routes.md`, `documentation/organisation-app.md`, `documentation/organisation-invites.md`, `documentation/organisation-members.md`, `documentation/fleet.md`, `documentation/flights.md`, `documentation/landing-page.md`.
